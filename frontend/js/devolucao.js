...
document.getElementById('confirmarDevolucao').addEventListener('click', confirmarDevolucao);
...
function startScanner() {
    ...
    Quagga.onDetected(function(data) {
        ...
        console.log('=== DEBUG SCANNER DEVOLUÇÃO ===');
...
async function buscarInstancia(idInstancia) {
    ...
    try {
        ...
        if (instancia.situacao !== 'emprestado') {
            throw new Error('Este livro não está emprestado e não pode ser devolvido');
        }
...
function exibirInformacoes(instancia, livro) {
    ...
    document.getElementById('posicaoAtual').textContent = `${instancia.posX}m`;
    ...
    // Sugerir posição próxima da atual
    const novaPosicaoInput = document.getElementById('novaPosicao');
    const posicaoSugerida = Math.round((instancia.posX + 0.1) * 10) / 10; // Arredondar para 1 casa decimal
    novaPosicaoInput.value = posicaoSugerida;
...
async function confirmarDevolucao() {
    ...
    const novaPosX = parseFloat(document.getElementById('novaPosicao').value);
    
    if (isNaN(novaPosX) || novaPosX < 0 || novaPosX > 10) {
        showError('Por favor, insira uma posição válida entre 0 e 10 metros');
        return;
    }
    ...
    try {
        const response = await fetch(`/instancias/${currentInstancia.idInstancia}/devolver`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.detail || 'Erro ao devolver livro');
        }
